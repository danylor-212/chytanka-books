...
- **CI і ліміти GitHub Actions:** зібрані книжки лежать у релізі-«насінні» (`en-cache-<дата>`, файл `en-cache.tar`,
  створюється `tools/make_en_seed.sh`); воркфлоу спершу розпаковує його, потім кеш попередніх запусків, і
  перезбирає лише книжки, чий коміт у SE змінився — не більше `SE_MAX_REBUILDS` (12) за запуск, кожну з лімітом
  `SE_BUILD_TIMEOUT` (30 хв); решта лишається з попередньою збіркою до наступного запуску. Job — `timeout-minutes: 300`.
